...
export default {
  ...
  computed: {
    posts() {
      ...
      //   }
      for (item in this.$store.state.posts) {
        if (this.$store.state.posts[item].user_id == this.user.id){
          var post = {
              ID: this.$store.state.posts[item].id,
              Tag: this.$store.state.posts[item].tag,
              User: this.$store.state.posts[item].user_name,
              Created:
                  this.$store.state.posts[item].created_at.slice(11, 16) +
                  ", " +
                  this.$store.state.posts[item].created_at.slice(0, 10), //slicing used to format timestamp as desired
