...
const chunkUploader = (endpoint, file, user, options) => {
    ...
    return api.post(endpoint, params)
        .then(res => {
            ...
            if (end === file.size) {
                ...
                options.chunkNumber++;
                return chunkUploader(endpoint, file, user, options);
            }
        }).catch(err => {
...
export default {
    chunk: (endpoint, file, user, onProgress, onError, onSuccess) => {
        const blockCount = Math.ceil(file.size / chunkSize);
        const chunkNumber = 0;
        const identifier = `${file.size}-${file.name.replace('.', '')}`;

        return chunkUploader(endpoint, file, user, {
            blockCount,
            identifier,
            chunkNumber,
            onProgress,
